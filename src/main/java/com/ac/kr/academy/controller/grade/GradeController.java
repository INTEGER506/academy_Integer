...
public class GradeController {
    ...
    public String professorCourses(@RequestParam(required = false) Long professorId,
                                   @RequestParam(required = false) String searchType,
                                   @RequestParam(required = false) String searchKeyword,
                                   @ModelAttribute PageRequestDTO req,
        ...
        // 로그인한 사용자의 professorId 사용 (파라미터가 없거나 권한 확인용)
        Long actualProfessorId = professorId;
        if (actualProfessorId == null) {
            // 실제로는 userDetails에서 professorId를 가져와야 함
            actualProfessorId = 1L; // 임시값
        }
        
        // CourseService를 활용하여 교수가 개설한 강의 목록 조회
        PageResponseDTO<Map<String, Object>> result = 
                gradeService.listProfessorCoursesFromCourseService(actualProfessorId, searchType, searchKeyword, req);

        model.addAttribute("result", result);
        model.addAttribute("req", req);
        model.addAttribute("professorId", actualProfessorId);

        Map<String, Object> keep = new HashMap<>();
        keep.put("professorId", actualProfessorId);
        model.addAttribute("keepParams", keep);
    ...
    public String ListForProfessor(@RequestParam Long professorId,
        ...
        if (result.getTotalCount() == 0) log.warn("성적 데이터 없음");
    ...
    public String listSemesterGrades(@PathVariable Long studentId, 
        ...
        // 로그인한 사용자의 학생 ID 사용
        Long actualStudentId = studentId; // URL의 studentId를 기본값으로 사용
        if (userDetails != null) {
            // 실제로는 userDetails에서 studentId를 가져와야 함
            actualStudentId = 1L; // 임시값
        }
        log.info("URL studentId: {}, 실제 사용할 studentId: {}", studentId, actualStudentId);
    ...
    // UserService를 활용한 학생 정보 조회
    private Map<String, Object> getStudentInfoFromDB(Long studentId) {
        log.info("🔍 학생 정보 조회 시작 - studentId: {}", studentId);
        
        try {
            // GradeService를 통해 학생 정보 조회
            log.info("📞 GradeService를 통해 학생 정보 조회 - studentId: {}", studentId);
            Map<String, Object> studentInfo = gradeService.getStudentInfoForGrade(studentId);
            
            if (studentInfo != null && !studentInfo.isEmpty()) {
                log.info("✅ GradeService로 학생 정보 조회 성공: {}", studentInfo);
                return studentInfo;
            } else {
                log.warn("❌ GradeService로 학생 정보를 찾을 수 없습니다 - studentId: {}", studentId);
                // 기본값 설정
                Map<String, Object> defaultStudentInfo = new HashMap<>();
                defaultStudentInfo.put("studentId", studentId);
                defaultStudentInfo.put("studentName", "학생정보없음");
                defaultStudentInfo.put("studentNo", "-");
                defaultStudentInfo.put("email", "-");
                defaultStudentInfo.put("phone", "-");
                defaultStudentInfo.put("major", "-");
                defaultStudentInfo.put("admissionYear", "-");

...
//    @Transactional
    ...
    public void updateUserBasicInfo(MyPageInfoDTO myPageInfoDTO){
        User user = new User();
        user.setId(myPageInfoDTO.getUserId());
    ...
    //사용자 ID, role로 상세 정보 조회 - 마이페이지 / MyPageInfoDTO 반환
    @Transactional(readOnly = true)
    public MyPageInfoDTO findMyPageInfo(Long userId, String role){
        switch (role){
            case "ROLE_STUDENT":
                return studentMapper.findByUserIdWithDept(userId);
            case "ROLE_PROFESSOR":
            case "ROLE_ADVISOR":
                return professorMapper.findByUserIdWithDept(userId);
            case "ROLE_STAFF":
                return staffMapper.findByUserIdForUpdate(userId);
            case "ROLE_ADMIN":
                //관리자는 roleEntity가 없으므로 User 정보만 DTO로 변환하여 반환
                User adminUser = userMapper.findById(userId);
